...
export default function PolicyLinks() {
  return (
    <section
      className="py-20 px-6 gold-border-top"
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-10">
          {policies.map(({ icon: Icon, title, desc, href }) => (
            <Link
              key={title}
              href={href}
              className="group block p-6 transition-all hover:opacity-95"
              style={{
                border: "1px solid rgba(200,146,26,0.15)",
                background: "rgba(82,5,21,0.3)",
              }}
            >
              <Icon
                size={20}
                style={{ color: "var(--gold-500)", marginBottom: "0.75rem" }}
              />
              <h3
                style={{
                  fontFamily: "var(--font-accent)",
        ...
        >
          ...
          <a
            ...
            }?text=Hi! I have a question about your policies.`}
            target="_blank"
