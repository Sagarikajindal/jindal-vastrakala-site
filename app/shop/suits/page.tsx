...
export default function SuitsPage() {
  const suits = getProductsByCategory("suit");
  return (
    <div className="min-h-screen pt-24 pb-24 px-6 max-w-7xl mx-auto">
      <div className="text-center mb-16">
        ...
        <h1 className="font-display text-gold-100 text-6xl font-light">Suits</h1>
        <p className="text-gold-500 font-body italic mt-3 text-lg">
          Anarkali · Churidar · Palazzo · Sharara
        </p>
        <div className="ornament mt-4 text-gold-600 text-sm" />
      </div>
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
        {suits.map((p) => <ProductCard key={p.id} product={p} />)}
      </div>
      {suits.length === 0 && (
